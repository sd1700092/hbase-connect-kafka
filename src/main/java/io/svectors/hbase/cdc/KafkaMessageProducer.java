...
public final class KafkaMessageProducer implements Closeable {
    ...
    public void send(final String tableName, final HRow row) {
        final ProducerRecord<byte[], String> record = new ProducerRecord<>(tableName, row.getRowKey(), JSON.toJSONString(row));
        if(this.isAsync) {
            sendAsync(record);
        } else {
            sendSync(record);
     ...
     * @param record
     */
    private void sendAsync(ProducerRecord<byte[], String> record) {
        boolean retry = true;
        while(retry) {
            try {
     ...
     * @param record
     */
    private void sendSync(ProducerRecord<byte[], String> record) {
        boolean retry = true;
        while(retry) {
            try {
